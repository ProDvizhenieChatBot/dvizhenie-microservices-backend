...
**[TODO](Epic) Задача 9.3: Написать автотесты для `file-storage-service`**
*   **Описание:** Создать набор автоматических тестов для `file-storage-service`, используя `pytest` и `moto` для мокирования S3-совместимого хранилища (MinIO).
*   **Подзадачи:**
    1.  **[TODO]** Настроить тестовое окружение с `moto` для эмуляции S3.
    2.  **[TODO]** Написать интеграционные тесты для эндпоинта загрузки файла (`POST /api/v1/files/`), проверяя успешное сохранение и корректный ответ.
    3.  **[TODO]** Написать интеграционные тесты для эндпоинта генерации ссылки на скачивание (`GET /api/v1/files/{file_id}/download-link`), проверяя успешное создание ссылки и обработку ошибки 404 для несуществующих файлов.
...
### **Фаза 7: Интеграция и финальная настройка**

**[TODO] Задача 10.1: Настроить CORS для интеграции с фронтендом**
*   **Описание:** Раскомментировать и правильно сконфигурировать `CORSMiddleware` в `api-service` и `file-storage-service`, чтобы разрешить запросы от фронтенд-приложения (Mini App или веб-виджета). Это необходимо для того, чтобы браузер не блокировал запросы из-за политики безопасности Same-Origin Policy.
*   **Подзадачи:**
    1.  **[TODO]** Получить от фронтенд-команды точный список URL-адресов (origins), используемых для разработки и продакшена.
    2.  **[TODO]** В `api-service/src/app/main.py` раскомментировать `CORSMiddleware` и заменить `allow_origins=['*']` на полученный список URL.
    3.  **[TODO]** В `file-storage-service/src/app/main.py` раскомментировать `CORSMiddleware` и настроить его аналогичным образом.
    4.  **[TODO]** Убедиться, что опция `allow_credentials=True` включена, чтобы обеспечить возможность отправки cookies или токенов авторизации в будущем.
*   **Критерии готовности:** Фронтенд-приложение может успешно выполнять запросы к `api-service` и `file-storage-service` без ошибок CORS. Настройки безопасности соответствуют лучшим практикам (не используется `allow_origins=['*']`).
---
