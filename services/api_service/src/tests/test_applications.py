...
class TestApplicationEndpointFunctions:
    ...
    @patch('app.api.applications.select')
    async def test_save_application_progress_unicode_data(self, mock_select):
        ...
        # Unicode test data
        unicode_data = {
            'name': 'Имя с ёмкими символами',
            'description': 'Special chars: @#$%^&*() и эмодзи 😊🎉',
            'unicode_test': '测试中文 العربية עברית русский',
        }

        application_update = ApplicationUpdate(data=unicode_data)
