...
Проект включает наборы автоматических тестов для `api-service` и `bot-service` с использованием `pytest` для обеспечения стабильности и предотвращения регрессий.

#### Запуск тестов

Процесс запуска тестов одинаков для обоих сервисов.
...
2.  **Установите зависимости для разработки:**
    Эта команда установит все зависимости, включая `pytest` и другие инструменты.
    ```bash
    uv sync --with ".[dev]"
    ```
...
4.  **Запустите тесты с отчетом о покрытии:**
    ```bash
    ...
    >
    > *   `uv sync` + `uv run pytest` — это двухэтапный процесс, удобный для локальной работы.
    > *   `uv run --with ".[dev]" pytest` является самодостаточной командой. Она автоматически установит `dev`-зависимости, если их нет в окружении, перед запуском тестов. Этот подход используется в CI/CD и pre-commit хуках для гарантии того, что нужные инструменты всегда доступны.
...
Для запуска тестов в CI/CD пайплайнах (например, GitHub Actions) можно использовать следующую конфигурацию для каждого сервиса:
...
- name: Upload api-service coverage to Codecov
...
- name: Upload bot-service coverage to Codecov
  uses: codecov/codecov-action@v4
  with:
    token: ${{ secrets.CODECOV_TOKEN }}
    files: ./services/bot_service/coverage.xml
    flags: bot-service
```
