...
*   `/start` — основная команда для начала работы. Бот приветствует пользователя, создает (или возобновляет) сессию заявки через `api-service` и присылает кнопку для открытия веб-приложения (Mini App) с анкетой.
